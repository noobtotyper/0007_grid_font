...
class SymbolDrawerSmooth:
    def __init__(self,stroke_semiwidth=math.sqrt(5)/20,corners=True, rgb=(0, 0, 0), bg=(0, 0, 0, 0)):
        self.stroke_semiwidth=stroke_semiwidth
        print(stroke_semiwidth)
        # a s d f are the junction points
        self.a = stroke_semiwidth
        self.s = 0.5-stroke_semiwidth
        self.d = 0.5+stroke_semiwidth
        self.f = 1-stroke_semiwidth
        # q e are the x or y positions (both) of the intersection of the circle with the long diagonals
        self.q=0.5-self.stroke_semiwidth/math.sqrt(2)
        self.e=0.5+self.stroke_semiwidth/math.sqrt(2)
        self.corners=corners
        self.rgb=rgb
        self.bg=bg
    
    def change_stroke_semiwidth(self,stroke_semiwidth=math.sqrt(5)/20):
        """
        0 <= stroke_semiwidth <= 1 is allowed\n
        0 < stroke_semiwidth < math.sqrt(5)/20 ~ 0.1118 is best (protodiagonal and center separate)\n
        math.sqrt(5)/20 < stroke_semiwidth < 1/4 is bad but readable\n
        beyond that it is unreadable\n
        
        Default value has a nice tangent of center with protodiagonals
        """
        assert type(stroke_semiwidth)==float and 0<=stroke_semiwidth<=1
        self.stroke_semiwidth=stroke_semiwidth
    ...
    # t=top, c=center, b=bottom, l=left, r= right, tl=topleft, ...
    ...
    def t_c(self, cr: cairo.Context):
        cr.move_to(self.s, 0)
        cr.line_to(self.d, 0)
        cr.line_to(self.d, 0.5)
        cr.line_to(self.s, 0.5)
        cr.close_path()
        cr.fill()
        self.fill_center(cr)
    def l_c(self, cr: cairo.Context):
        cr.move_to(0, self.s)
        cr.line_to(0, self.d)
        cr.line_to(0.5, self.d)
        cr.line_to(0.5, self.s)
        cr.close_path()
        cr.fill()
        self.fill_center(cr)
    def b_c(self, cr: cairo.Context):
        cr.move_to(self.s, 0.5)
        cr.line_to(self.d, 0.5)
        cr.line_to(self.d, 1)
        cr.line_to(self.s, 1)
        cr.close_path()
    ...
    def tl_c(self, cr: cairo.Context):
        ...
        cr.line_to(0, 0)
        cr.line_to(self.a, 0)
        cr.line_to(self.e, self.q)
        cr.line_to(self.q, self.e)
        cr.close_path()
        cr.fill()
        self.fill_center(cr)
    def tr_c(self, cr: cairo.Context):
        ...
        cr.line_to(1, 0)
        cr.line_to(1, self.a)
        cr.line_to(self.e, self.e)
        cr.line_to(self.q, self.q)
        cr.close_path()
        cr.fill()
        self.fill_center(cr)
    ...
    ## center
    def fill_center(self, cr: cairo.Context):
        cr.arc(0.5,0.5,self.stroke_semiwidth,0,2*math.pi)
        cr.fill()
    ...
    def fill_symbol(self, cr: cairo.Context, symbol):
        match symbol:
            ...
            case "=":  # _equal
                ...
                cr.line_to(0.75, 0.25+2*self.stroke_semiwidth)
                cr.line_to(0.25, 0.25+2*self.stroke_semiwidth)
                cr.close_path()
                cr.move_to(0.75,0.75)
                cr.line_to(0.25,0.75)
                cr.line_to(0.25,0.75-2*self.stroke_semiwidth)
                cr.line_to(0.75,0.75-2*self.stroke_semiwidth)
                cr.close_path()
                cr.fill()
            case "€":  # _euro
                self.l_tr(cr)
            ...
            case "×":  # _multiplication
                ...
                cr.line_to(0.25, 0.75)
                cr.set_line_width(self.stroke_semiwidth*math.sqrt(2))
                cr.stroke()
            case "¬":  # _not
                self.l_c(cr)
                self.c_b(cr)
